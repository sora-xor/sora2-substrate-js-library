...
export class PoolXykModule {
   ...
   */
   public getInfo (firstAssetAddress: string, secondAssetAddress: string): Asset {
    const poolTokenAccount = poolAccountIdFromAssetPair(this.root.api, firstAssetAddress, secondAssetAddress).toString()
    if (!poolTokenAccount) {
      return null
    }
   ...
   */
   public async check (firstAssetAddress: string, secondAssetAddress: string): Promise<boolean> {
    const props = (await this.root.api.query.poolXyk.properties(firstAssetAddress, secondAssetAddress)).toJSON() as Array<string>
   ...
   */
   public updateAccountLiquidity (targetAssetIds: Array<string>): void {
    ...
    for (const liquidity of liquidityList) {
      const subscription = this.root.apiRx.query.poolXyk.reserves(XOR.address, liquidity.secondAddress).subscribe(async reserves => {
  ...
  private async calcCreateTxParams (
    ...
    const params = await this.calcAddTxParams(baseAsset, targetAsset, baseAssetAmount, targetAssetAmount, slippageTolerance)
    return {
      pairCreationArgs: [
        this.root.defaultDEXId,
        baseAsset.address,
   ...
   */
   public async create (
    ...
    const isPairAlreadyCreated = (await (this.root.api.rpc as any).tradingPair.isPairEnabled(this.root.defaultDEXId, firstAsset.address, secondAsset.address)).isTrue as boolean
    const transactions = []
