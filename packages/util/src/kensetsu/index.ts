...
export class KensetsuModule<T> {
   ...
   * Usage: general system parameters, statistical information
   */
  subscribeOnBadDept(): Observable<number> {
    return this.root.apiRx.query.kensetsu.badDebt().pipe(map((res) => res.toNumber()));
  }
   ...
   */
  subscribeOnKusdHardCap(): Observable<number> {
    return this.root.apiRx.query.kensetsu.kusdHardCap().pipe(map((res) => res.toNumber()));
  }
